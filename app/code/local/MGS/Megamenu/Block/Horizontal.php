...
class MGS_Megamenu_Block_Horizontal extends MGS_Megamenu_Block_Abstract
{
	...
	public function getClass($item){
		...
		if($item->getColumns()>1){
			$class.= " mega-menu-item mega-menu-fullwidth level0";
		}
		if($type==2){
	...
	public function getStaticMenu($item){
		...
		if($item->getStaticContent()!=''){
			...
			$html .= '<div class="mega-content-wrap">';
	...
	public function drawListSub($item, $catIds){
		...
		if($item->getColumns()>1){
			$html.='<div class="col-md-'.$this->getColumnByCol($item->getColumns()).' col-sm-12 col-xs-12 block"><ul class="sub-menu inner">';
		}
