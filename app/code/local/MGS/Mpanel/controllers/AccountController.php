...
class MGS_Mpanel_AccountController extends Mage_Core_Controller_Front_Action {
    ...
    public function applyToAllAction() {
        ...
        if ($data = $this->getRequest()->getParams()) {
            try {
                $pageId = $data['page_id'];
                $tempCollection = Mage::getModel('mpanel/block')
                        ->getCollection()
                        ->addFieldToFilter('page_type', array('eq' => 'account'))
                        ->addFieldToFilter('page_id', array('eq' => 0))
                        ->setOrder('sort_order', 'ASC');
                $blocksData = array();
                foreach ($tempCollection as $block) {
                    $blocksData[] = array(
                        'page_type' => $block->getData('page_type'),
                        'page_id' => $block->getData('page_id'),
                        'place' => $block->getData('place'),
                        'type' => $block->getData('type'),
                        'title' => $block->getData('title'),
                        'options' => $block->getData('options'),
                        'sort_order' => $block->getData('sort_order')
                    );
                }
